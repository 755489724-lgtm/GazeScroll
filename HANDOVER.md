...
> **⚠️ 2026-09-17 补充（v5.39 → v5.40）**：工作区现在**不是** v5.36，而是
> **v5.40 = v5.36 + 「注视数据采集」测试功能**（默认关闭、只记录不判定，
> 不开时逐帧开销与 v5.36 完全一致 → 行为等同锚点）。锚点仍是 **v5.36**，
> 一键装回用 `tools\install-v536.ps1`。细节见 `CHANGELOG.md` 的 `[5.40]` / `[5.39]` 两节、
> `README.md` 的「注视数据采集（v5.39，测试功能）」一节、
> `backup\GazeScroll-v5.40\ROLLBACK.txt`。
> 版本号之所以是 5.39/5.40：**v5.37 / v5.38 已被上一轮那次失败实验占用**（tag / backup / apk / 文档条目）。

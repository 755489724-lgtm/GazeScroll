...
> ### 📌 v5.51 / v5.60 补充（同一天更晚，**版本号以这里为准**）
>
> - **v5.51**：二级菜单的三角放大（纯 UI，两个 XML）。
> - **v5.60**：**四个触发参数交给用户自己调**（用户点名）——
>   点头角度 / **仰头角度（新拆出来的独立值）** / 扭头角度 / 歪头角度 全部做成**滑块**，
>   外加**「触发速度」第一次露出 UI**（150~1500ms），每项旁边一个「默认」按钮。
>   **默认值 = v5.51 交付时他实际在用的那一套**：`GazeModel.RESET_*` = 6 / 6 / 28 / 13 / 500。
>   - 新增配置键 **`headPoseUpThresholdDeg`**：**缺失时直接跟随 `headPoseAngleThreshold`**，
>     所以**老配置升级上来行为一字不变**（实测 `thCfg=6.0/6.0°`）。
>   - `HeadPoseDetector.thresholdUpDeg`：**`<=0` 表示跟随 `thresholdDeg`** ——
>     离线回放工具只设 `thresholdDeg` 时行为不变（**这条约定别改**）。
>   - 「触发速度」同时管两条轴，但扭头窗口 = 滑块值 × **1.8**
>     （`HeadPoseDetector.TURN_WINDOW_RATIO`），默认 500 → **900ms，与 v5.51 的写死值一字不差**
>     （日志实测 `winCfg=500/900ms`）。改这个比例前先想清楚"默认档必须等于旧行为"。
>   - 诊断行新增可核对字段：**`thCfg=` / `winCfg=` / `yawThCfg=` / `tiltThCfg=`**。
>   - 离线回放（`tools/headpose-replay`）扩到 **19 条**：原 10 条默认路径 + 9 条新用例
>     （仰头独立、点头不受影响、比例换算、触发速度管到扭头）。
>   - ⚠️ **仍然成立的老结论**：30cm 那次点头误触（4.9~8°）与他有意的轻点头（3.0~4.4°）**完全重叠**，
>     调大「点头角度」会一起废掉轻点头。旋钮交给他只是让他自己找平衡点，**不是"调大就好了"**。

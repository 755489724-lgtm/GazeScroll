...
> **⚠️ 2026-09-17 补充（v5.39 → v5.43）**：工作区现在**不是** v5.36，而是
> **v5.43 = v5.36 + 「注视数据采集」（测试功能）+ 【注视门】**。注视门默认是
> **「观察」模式（只记录、不拦）**，所以默认状态下行为仍与 v5.36 一致；
> 用户在设置页拨到「拦截」才真的拦。锚点仍是 **v5.36**，一键装回用 `tools\install-v536.ps1`。
> 细节见 `CHANGELOG.md` 的 `[5.43]`~`[5.39]` 各节、`README.md` 的「注视门」「注视数据采集」两节、
> `backup\GazeScroll-v5.43\ROLLBACK.txt`。
> 版本号之所以是 5.39~5.43：**v5.37 / v5.38 已被上一轮那次失败实验占用**（tag / backup / apk / 文档条目）。

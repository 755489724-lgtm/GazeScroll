...
private fun testReturnToNeutralDoesNotFire() {
    println("\n[5] 歪着不动停一会再回正：回正不能触发（自愈：2.5 秒后基准线重锚到当前头姿）")
    val f = Feeder()
    f.feed(0f, 1500)
    f.feed(20f, 3000)               // 歪 20° 停 3 秒（0.3s 就触发；2.5s 后判定为"头姿"并重锚）
    ...
    f.feed(0f, 4200)                // 在中位停稳 → 基准线重锚回当前头姿（重锚后再等满 2 秒）
    check(
        "自愈：静止后倾斜角回到 0 附近",
    ...
    check("自愈后重新歪照常触发", f.events.size == 2, "触发 ${f.events.size} 次")
}

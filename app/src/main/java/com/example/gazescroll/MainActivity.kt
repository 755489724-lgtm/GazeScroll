...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * v5.75：「省电模式（没动作时自动降档）」开关 + 等待时长滑块。
     *
     * 放在「防误触」卡的二级菜单里 —— 它和静止防误触是同一类东西：
     * 都是"在你没动作的时候，替你把灵敏度/功耗收一收"。
     */
    private fun setupIdleThrottleUi() {
        // 档数来自 GazeConfig 常量，不写死在 XML 里（那踩过坑：滑块会永远拖不到上限而静默失效）。
